...
import { ArrowLeft, ArrowRight, Ban, Cloud, Network, RefreshCcw, ServerCog, Trash } from "lucide-react";
...
export default function ManageView() {
    ...
    const { getClientID, getSessionToken } = useUtils()
    const router = useNavigate()
    const { currentPlan } = useClientStore()
    ...
    useEffect(() => {
        const service = { id: currentPlan.id, type: "credencialServicoHospedagem" }
        ...
        <div className="w-full h-full overflow-y-scroll">
            <div className="flex items-center justify-between w-full h-[80px] my-[15px]">
                <div className="flex flex-col items-start justify-center">
                    <h1 className="text-[#616161] font-[400] text-[2rem] font-[Rubik]" style={{ lineHeight: 1 }}>Serviço de Hospedagem</h1>
                    <p className="font-[Rubik] mt-2 opacity-70">Domínio: <span className='bg-[#0d528022] ml-1 py-1.5 px-2.5 text-[0.8rem] rounded-full'>{currentPlan.dominio}</span></p>
                </div>
            </div>
            <div className="flex items-center justify-between w-full h-[80px]">
                <div className="flex items-center justify-center gap-2">
                    <div className="p-2 bg-green-100 text-green-700 rounded-[12px] border-[1px] border-solid border-green-600 border-opacity-40">
                        <Cloud strokeWidth={1.5} size={30} />
                    </div>
                    <div>
                        <p style={{ lineHeight: '10px' }} className="font-[Rubik] mt-2 opacity-90 text-[1.2rem]">{currentPlan.Plano.titulo}</p>
                        <p className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">ID do Serviço: {currentPlan.id}</p>
                    </div>
                </div>
                <div className="flex items-center w-max h-max justify-center">
                    <div >
                        <p style={{ lineHeight: '13px' }} className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">Inicio: {formatDate(currentPlan.atualizadoEm)}</p>
                        <p style={{ lineHeight: '13px' }} className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">Termino: {formatDate(currentPlan.expiraEm as string)}</p>
                    </div>
            ...
            <div className="w-full flex gap-[30px] items-start pt-[30px] justify-center" style={{ height: 'calc(100vh - 250px)' }}>
                <div style={{ width: 'calc(100% - 300px)' }}>
                    <div className="h-max bg-white flex flex-col gap-4 rounded-[18px] border-solid border-[1px] p-12 items-center justfy-center" >
                        <div className="w-full h-max flex gap-4">
                            <div className="w-full bg-[#edf3f8] rounded-[20px] flex items-center justify-center px-2">
                                <div className="flex flex-col py-2 items-center justify-center ">
                                    <Cloud size={30} color="#135183" strokeWidth={1.3} />
                                    <div className="w-full flex flex-col pb-2 items-start justify-center mt-1 gap-2">
                                        <h1 style={{ lineHeight: '20px' }} className="text-[1rem] text-center text-[#135183] font-semibold">Gerenciar hospedagem</h1>
                                        <button onClick={() => {
                                            setLoading(true)
                                        ...
                                        }} disabled={loading} className="h-[33px] w-[160px] mx-auto text-[#0e324f8b] rounded-md bg-[#0d528022] flex items-center gap-1 justify-center text-white text-[0.8rem]" style={{ background: 'linear-gradient(90deg, #2160ad, #082140)' }}>{loading ? <TailSpin color="#fff" width={12} /> : <>Entrar no CPanel <ArrowRight className="inline" size={17} strokeWidth={1.4} /></>}</button>
                                        <button onClick={openCPanelModal} style={{ lineHeight: '0px' }} className="text-[0.8rem] mx-auto mt-1.5 text-[#0e324f8b]">Ver detalhes </button>
                                    </div>
                                </div>
                            </div>
                            <div className="w-full bg-[#edf3f8] rounded-[20px] flex items-center justify-center px-2">
                        ...
                        {/* <div className="w-full h-max flex gap-4">
                            <div className="w-full bg-[#edf3f8] rounded-[20px] flex items-center py-4 justify-center px-6">
                                <div className="flex items-center justify-center ">
                                    <Settings size={30} color="#135183" strokeWidth={1.3} />
                                    <div className="w-full flex flex-col items-start justify-center mt-2 ml-3 gap-1">
                                        <h1 style={{ lineHeight: '20px' }} className="text-[1.1rem] text-[#135183] font-semibold">Alterar dominio</h1>
                                        <p style={{ lineHeight: '0px' }} className="text-[0.8rem] text-[#0e324f8b]">Ver detalhes <ArrowRight className="inline ml-2" size={18} /></p>
                                    </div>
                                </div>
                            </div>
                        </div> */}
                    ...
                    <div className="flex items-center justify-start gap-2 mt-2">
                        <button onClick={() => { router('/cliente/painel/hospedagem/servicos') }} className="py-2 hover:shadow-sm hover:-translate-y-0.5 transition-all duration-300 mb-2 px-3 rounded-md bg-white text-gray-600 text-[0.8rem] flex items-center justify-center gap-2 font-medium"><ArrowLeft size={14} /> Ver outros serviços </button>
                ...
                <div className="w-[270px]">
                    <p className="font-[Rubik] mt-2 opacity-70 text-[1rem]">Resumo das atividades <span className=" ml-2 py-1.5 px-4 bg-red-100 text-red-800 rounded-full">Beta</span></p>
