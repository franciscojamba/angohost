...
export default function DomainManageView() {

    const { id } = useParams()
    const { openCPanelModal, openDNSModal, openIPModal } = useModalsStore()
    const [loading, setLoading] = useState(false)
    const router = useNavigate()
    const { currentDomainService } = useClientStore()
    ...
    useEffect(() => {
        const service = { id: id as string, type: "credencialServicoDominio" }
        ...
        <div className="w-full h-full overflow-y-scroll">
            <div className="flex items-center justify-between w-full h-[80px] my-[15px]">
                <div className="flex flex-col items-start justify-center">
                    <h1 className="text-[#616161] font-[400] text-[2rem] font-[Rubik]" style={{ lineHeight: 1 }}>Serviço de domínio </h1>
            ...
            <div className="flex items-center justify-between w-full h-[80px]">
                <div className="flex items-center justify-center gap-2">
                    ...
                    <div>
                        <p style={{ lineHeight: '10px' }} className="font-[Rubik] mt-2 opacity-90 text-[1.2rem]">{currentDomainService.Dominio.dominio}</p>
                        <p className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">ID do Serviço: {currentDomainService.id}</p>
                    </div>
                </div>
                <div className="flex items-center w-max h-max justify-center">
                    <div >
                        <p style={{ lineHeight: '13px' }} className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">Inicio: {formatDate(currentDomainService.atualizadoEm)}</p>
                        <p style={{ lineHeight: '13px' }} className="font-[Rubik] mt-2 opacity-70 text-[0.8rem]">Termino: {formatDate(currentDomainService.expiraEm as string)}</p>
                    </div>
            ...
            <div className="w-full flex gap-[30px] items-start pt-[30px] justify-center" style={{ height: 'calc(100vh - 250px)' }}>
                <div style={{ width: 'calc(100% - 300px)' }}>
                    ...
                    <div className="flex items-center justify-start gap-2 mt-2">
                        <button onClick={()=>{router('/cliente/painel/dominios/servicos')}} className="py-2 hover:shadow-sm hover:-translate-y-0.5 transition-all duration-300 mb-2 px-3 rounded-md bg-white text-gray-600 text-[0.8rem] flex items-center justify-center gap-2 font-medium"><ArrowLeft size={14} /> Ver outros serviços </button>
