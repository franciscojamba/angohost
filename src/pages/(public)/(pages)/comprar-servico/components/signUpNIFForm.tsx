...
interface ISignUpNIFFormProps {
    location: {
        pais: string,
        provincia: string
    },
    submtiNIF: (data: formType) => void,
}

export default function SignUpNIFForm({location, submtiNIF}: ISignUpNIFFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit(submtiNIF)} className='formNIF w-full'>
            <div className='flex gap-3 w-full'>
                <div className='flex flex-col w-full lg:w-max h-max'>
                    <label htmlFor="" className='font-semibold text-[0.9rem] text-[#222]'>Nome da empresa*</label>
            ...
            <button className='w-[0px] h-[0px] submitNIF opacity-0'> </button>
        </form>
    )
}